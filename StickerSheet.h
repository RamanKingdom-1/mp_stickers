...
private:
  ...
  // You MUST use a std::vector to store the sticker layers

  Image* holder;
  Image original_picture; 
  std::vector<Image*> layers_;
  std::vector<std::vector<int>> location;
  mutable int width_in_case_of_negative;
  mutable int height_in_case_of_negative;
  mutable int width_in_case_of_extra;
  mutable int height_in_case_of_extra;
...
public:
  ...
  void negative_values(Image& currently,int width_, int height_)const;

  void positive_values(Image& currently,int width_, int height_)const;

  void render_helper(Image& currently,int x, int y)const;

  void copy_original_picture();
